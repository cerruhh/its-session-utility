...
// New function: split all JSON files in a folder
void split_json_folder(const std::string& folder_path, int chunk_size) {
    for (const auto& entry : fs::directory_iterator(folder_path)) {
        if (entry.is_regular_file() && entry.path().extension() == ".json") {
            std::cout << "Rechunking " << entry.path() << "...\n";
            split_json_messages(entry.path().string(), chunk_size);
        }
    }
}
...
int main(int argc, char* argv[]) {
    ...
    if (chunk_size <= 0 || export_path.empty()) {
        std::cerr << "Usage: " << argv[0] 
                  << " --chunk-size <size> --export-path <file_or_folder> [--folder]\n"
                  << "  -s, --chunk-size   Number of messages per chunk\n"
                  << "  -e, --export-path  Path to JSON file or folder\n"
                  << "  -f, --folder       Treat export-path as folder and rechunk all JSONs inside\n";
        return 1;
    }

    if (folder_mode) {
        split_json_folder(export_path, chunk_size);
    } else {
